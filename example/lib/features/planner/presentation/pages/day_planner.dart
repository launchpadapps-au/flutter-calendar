...
import 'package:edgar_planner_calendar_flutter/features/planner/presentation/widgets/dayview/day_cell.dart';
import 'package:edgar_planner_calendar_flutter/features/planner/presentation/widgets/dayview/day_event.dart';
import 'package:edgar_planner_calendar_flutter/features/planner/presentation/widgets/dayview/day_header.dart';
import 'package:edgar_planner_calendar_flutter/features/planner/presentation/widgets/dayview/day_hour_lable.dart';
...
class DayPlanner extends StatefulWidget {
  ...
  final bool Function(
      CalendarEvent<EventData> draggeed,
      CalendarEvent<EventData> existing,
...
class _DayPlannerState extends State<DayPlanner> {
  ...
  Widget build(BuildContext context) => NewSlDayView<EventData>(
      ...
      onWillAccept:
          (CalendarEvent<EventData> event, DateTime dateTime, Period period) =>
      ...
      headerDecoration: (DateTime dateTime) => const BoxDecoration(
          boxShadow: <BoxShadow>[BoxShadow(blurRadius: 3, offset: Offset(0, 2), color: Color(0x0000001A))]),
      hourLabelBuilder: (Period period) => DayHourLable(periodModel: period as PeriodModel, isMobile: widget.isMobile),
      controller: widget.timetableController,
      isCellDraggable: (CalendarEvent<EventData> event) => CalendarUtils.isCelldraggable(event),
      itemBuilder: (CalendarEvent<EventData> item, int index, int length, double width) => DayEvent(
            item: item,
      ...
      cellBuilder: (Period period, DateTime date) => DayCell(periodModel: period as PeriodModel, breakHeight: widget.timetableController.breakHeight, cellHeight: widget.timetableController.cellHeight, isMobile: widget.isMobile));
}
