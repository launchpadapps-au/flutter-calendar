...
import 'package:flutter_calendar/src/widgets/infinite_listview.dart';
...
class LinkedInfiniteScrollControllerGroup {
  ///initialize
  LinkedInfiniteScrollControllerGroup() {
    _offsetNotifier = _LinkedInfiniteScrollControllerGroupOffsetNotifier(this);
  }

  final List<_LinkedInfiniteScrollController> _allControllers =
      <_LinkedInfiniteScrollController>[];

  late _LinkedInfiniteScrollControllerGroupOffsetNotifier _offsetNotifier;

  /// The current scroll offset of the group.
  double get offset {
    assert(
      _attachedControllers.isNotEmpty,
      'LinkedInfiniteScrollControllerGroup does not have any scroll controllers'
      'attached.',
    );
  ...
  InfiniteScrollController addAndGet() {
    final double initialScrollOffset = _attachedControllers.isEmpty
        ? 0.0
        : _attachedControllers.first.position.pixels;
    final _LinkedInfiniteScrollController controller =
        _LinkedInfiniteScrollController(this,
  ...
  Iterable<_LinkedInfiniteScrollController> get _attachedControllers =>
      _allControllers.where((_LinkedInfiniteScrollController controller) =>
  ...
  Future<void> animateTo(
    ...
    for (final _LinkedInfiniteScrollController controller
...
/// This class provides change 
/// notification for [LinkedInfiniteScrollControllerGroup]'s
/// scroll offset.
///
/// This change notifier de-duplicates change events by only firing listeners
/// when the scroll offset of the group has changed.
class _LinkedInfiniteScrollControllerGroupOffsetNotifier
    extends ChangeNotifier {
  _LinkedInfiniteScrollControllerGroupOffsetNotifier(this.controllerGroup);

  final LinkedInfiniteScrollControllerGroup controllerGroup;
...
/// be a [_LinkedInfiniteScrollController].
class _LinkedInfiniteScrollController extends InfiniteScrollController {
  _LinkedInfiniteScrollController(this._controllers,
      {required double initialScrollOffset})
      : super(
            initialScrollOffset: initialScrollOffset, keepScrollOffset: false);
  final LinkedInfiniteScrollControllerGroup _controllers;
  @override
  void dispose() {
  ...
  void attach(ScrollPosition position) {
    assert(
        ...
        '_LinkedInfiniteScrollControllers can only be used with'
  ...
  Iterable<_LinkedInfiniteScrollController> get _allPeersWithClients =>
      _controllers._attachedControllers
          .where((_LinkedInfiniteScrollController peer) => peer != this);
  ...
  Iterable<_LinkedScrollActivity> linkWithPeers(_LinkedScrollPosition driver) {
    ...
    return _allPeersWithClients
        .map((_LinkedInfiniteScrollController peer) => peer.link(driver))
...
class _LinkedScrollPosition extends ScrollPositionWithSingleContext {
  ...
  final _LinkedInfiniteScrollController owner;
